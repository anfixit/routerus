...
panel_steps_tail() {
    ...
    echo ""
    echo "    а) Какие порты слушает Xray (самая надёжная проверка):"
    ...
    echo ""
    echo "    б) Что нода получила из панели при последней загрузке конфига:"
    echo "         docker logs remnawave-node --since 10m | grep '·'"
    echo "       Считай строки ПОСЛЕ последней записи о запуске: при каждом"
    echo "       перезапуске блок печатается заново, и без --since их накопится больше."
    echo ""
    if [[ "$TRANSPORT" == "both" ]]; then
        echo "    в) Порты инбаундов в панели должны РАЗЛИЧАТЬСЯ — 443 и ${XHTTP_PORT}."
        echo "       Открой список выбора inbound при создании хоста: там у каждого"
        echo "       показан порт. Оба по 443 — панель хранит устаревшие записи,"
        echo "       перевставь профиль целиком и проверь снова."
        echo ""
    fi
    echo "    г) Публичный ключ ноды — сверить с подпиской (ШАГ 6):"
    echo "         grep PUBLIC_KEY ${OPT_DIR}/keys.txt"
    echo ""
    echo "    д) Полная диагностика ноды одной командой:"
    echo "         sudo bash check-node.sh"
    echo ""
    echo "ШАГ 6. Проверка в клиенте"
    echo "    Обновить подписку → включить ноду → открыть любой сайт."
    echo ""
    echo "    ГЛАВНАЯ СВЕРКА: в ссылке подписки найди pbk= и сравни с ключом из"
    echo "    ШАГА 5г. Не совпало — хост в панели держит устаревшую запись"
    echo "    инбаунда: Reality не признаёт клиента и молча отдаёт его на лендинг."
    echo "    Снаружи это выглядит как полностью нерабочая нода, хотя нода цела."
    echo "    Лечится пересозданием хоста, а не переустановкой ноды."
    echo ""
    if [[ "$TRANSPORT" == "both" ]]; then
