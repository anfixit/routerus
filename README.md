...
- **Три режима транспорта** на выбор при запуске:
  ...
  - `xhttp` — маскировка под HTTP-запросы для обхода блокировки VLESS TCP;
...
> `"network": "tcp"` на `"network": "raw"` (в Xray 25.x это одно и то же).
...
без доработок. В режиме `both` подписка содержит и tcp-, и xhttp-ссылку —
приложения (Happ) видят обе, а роутер берёт только tcp (xhttp на mihomo
требует sing-box-extended). Раскатка подписки на роутер и авто-выбор
...
DPI) одновременно, с общим ключом. podkop через `podkop-sub` автоматически
берёт из подписки только tcp.
...
| Версия | Изменения                                                        |
|--------|------------------------------------------------------------------|
| v3.6   | Режим транспорта `both` (tcp+xhttp на одной ноде), авто-открытие xhttp-порта в UFW, корректная подсказка про авто-flow |
