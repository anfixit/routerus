...
Скрипт спросит: **домен**, **публичный SSH-ключ**, **имя ноды**, **транспорт**.
...
Хардинг, geo, nginx, SSH и ключи не затрагиваются. Чтобы зафиксировать
...
> при мёртвом VPN-инбаунде. После обновления проверь реальный inbound:
> `ss -tlnp | grep -E ':(443|8444)'` на ноде.
...
- Docker ставится из официального apt-репозитория с GPG-проверкой пакетов
  (не `curl … | sh`); проверяется наличие `docker compose` v2.
...
- прогони [ShellCheck](https://www.shellcheck.net/): `shellcheck deploy-remnanode.sh` — должно быть без замечаний;
